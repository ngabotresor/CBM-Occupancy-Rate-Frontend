...
const MarketRecord = () => {
  ...
  const dateTemplate = (props) => <span>{formatDate(props.created_at)}</span>;
  ...
  const filteredMarketRecords = marketrecord?.filter((record) => {
    if (currentUser.role === 'verifier') {
      return (
        record.status === 'pending' || record.status === 'rollback_to_be_signed'
      );
    ...
    <div className="m-2 md:m-2 mt-2 p-2 md:p-4 bg-white rounded-3xl dark:bg-secondary-dark-bg">
      <div className="flex justify-between items-center">
        ...
        <div className="flex">
          ...
          (currentUser.role == 'header') | (currentUser.role == 'admin') ? (
            <button
              onClick={() => {
                setProcessed(!proccessed);
              }}
              type="button"
          ...
          {currentUser.role == 'creator' && (
            ...
            >
              {' '}
              <MdAddShoppingCart /> <span className="ml-1"> Add New </span>
            </button>
          )}
        </div>
      </div>
      ...
      >
        <ColumnsDirective>
          ...
          {marketrecordGrid.map((item, index) => (
