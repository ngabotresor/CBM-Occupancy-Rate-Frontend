...
} from '../services/marketRecordService';
...
const MarketRecordDetail = () => {
  ...
  const { data: comments, isLoading: commentLoading } =
    useGetMarketRecordCommentsByMarketRecordIdQuery(
      marketRecord?.id ? marketRecord?.id : skipToken
    );
  ...
  const components = componentData?.components.filter(
    (component) => component.market == marketRecord.market
  );

  const [updateMarketRecordProgress, { isLoading: updateLoading }, error] =
    useUpdateMarketRecordProgressMutation();
  ...
  const handleAction = async () => {
    try {
      ...
      const result =
        action == 'forward'
          ? await updateMarketRecordProgress({
              id: marketRecord?.id,
              data: {
                action: act,
                comment: comment,
                viewers: selectedViewerIds,
              },
            })
          : await updateMarketRecordProgress({
              id: marketRecord?.id,
              data: { action: act, comment: comment },
            });
  ...
  const handleAddRecord = () => {
    const hasEmptyComponentName = marketRecord.records.some(
      (record) => record.component_name.trim() === ''
    );
    const hasValidNumbers = marketRecord.records.some(
      (record) =>
        !record.total_number_places_available > 0 ||
        !record.number_places_rented === ''
    );
    if (hasEmptyComponentName || hasValidNumbers) {
      message.error(
        'Provide data for components, number of places available and rented'
      );
      return;
    }
    if (
      marketRecord?.market.trim() === '' ||
      marketRecord?.season?.trim() === ''
    ) {
    ...
    if (
      marketRecord.records.some(
        (record) =>
          record.total_number_places_available < record.number_places_rented
      )
    ) {
      message.error(
        'Number of placess available can not be less that places rented'
      );
      return;
    }
  ...
  const [addMarketRecord, { isLoading: addLoading }] =
    useAddMarketRecordMutation();
  const [updateMarketRecord, { isLoading: editLoading }] =
    useUpdateMarketRecordMutation();
  ...
  const totalPlacesAvailable =
    Math.round(
      marketRecord?.records.reduce(
        (total, record) => total + record.total_number_places_available,
        0
      ) * 10
    ) / 10;
  const totalPlacesRented =
    Math.round(
      marketRecord?.records.reduce(
        (total, record) => total + record.number_places_rented,
        0
      ) * 10
    ) / 10;
  const totalOccupancyRate =
    Math.round(((totalPlacesRented * 100) / totalPlacesAvailable) * 10) / 10;
    ...
    <div className="m-2 md:m-2 mt-2 p-2 md:p-4 bg-white rounded-3xl dark:bg-secondary-dark-bg">
      ...
      {!editMarketRecord && !marketRecord?.id == '' ? (
        <div className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div>
              ...
              <p className="text-lg mb-4">
                {marketRecord?.season} - {marketRecord?.year}
              </p>

              <h2 className="text-xl font-bold mb-2">Created By:</h2>
              <p className="mb-4">
                {marketRecord?.created_by
                  ? `${marketRecord?.created_by?.firstname} ${marketRecord?.created_by?.lastname}`
                  : 'Unknown'}
              </p>
              <p className="text-xs text-blue-500">
                {' '}
                <span className="text-gray-700 font-bold">On: </span>{' '}
                {marketRecord?.created_at
                  ? `${formatDate(marketRecord?.created_at)}`
                  : ''}
              </p>
            </div>
            ...
            <div className="mb-8">
              ...
              <p>
                ...
                <span className="uppercase">{marketRecord?.status}</span>
              </p>
            ...
            <table className="w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {marketRecord?.records?.map((record, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-6 py-4 whitespace-no-wrap">
                      {Math.round(record.total_number_places_available * 10) /
                        10}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap">
                      {Math.round(record.number_places_rented * 10) / 10}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap">
                      {Math.round(record.occupancy_rate * 10) / 10}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap">
                      {record.observation}
                    </td>
              ...
              <tfoot>
                <tr>
                  ...
                  {/* Empty cell for the 'Observation' column */}
                </tr>
              </tfoot>
            </table>
          </div>
                ...
                <div className="border-2 border-color rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                  <div className="flex items-start justify-between p-5 border-b border-solid border-gray-300 rounded-t ">
                    <h3 className="text-3xl font=semibold text-center">
                      {action == 'rollback' ? 'Reject' : 'Approve'}
                    </h3>
                    <button
                      style={{
                  ...
                  <div className="shadow overflow-hidden sm:rounded-md">
                    <div className="px-4 py-5 bg-white sm:p-6">
                      <h3 className="mb-2">
                        ...
                        {action == 'rollback' ? 'Reject' : action}' this report
                      </h3>
          ...
          <div>
            <h2 className="font-bold mt-4 ">Action</h2>
            {currentUser.role?.toLowerCase() == 'creator' &&
            marketRecord?.status?.toLowerCase() == 'rollback' ? (
              <div className="border-t-1 p-2 border-b-1 items-center text-center">
                <div className="flex justify-between mx-4 ">
                  <p>
                    This report has been sent back, kindly read the comment
                  ...
                  >
                    {' '}
                    <span className="h-12 ml-1"> {'Edit >>'} </span>
                  </button>
                </div>
              </div>
            ) : currentUser.role?.toLowerCase() == 'verifier' &&
              (marketRecord?.status?.toLowerCase() == 'pending' ||
                marketRecord?.status?.toLowerCase() ==
                  'rollback_to_be_signed') ? (
              <div className="border-t-1 p-2 border-b-1 items-center text-center">
                <div className="p-2 mb-1">
                  Kindly verify or reject this report
                </div>
                <div className="flex justify-between mx-4 ">
                  <button
                    onClick={() => {
                      if (!comment || comment.trim().length < 5) {
                        message.error(
                          'Please provide a valid comment for rejection'
                        );
                      } else {
                        setAction('rollback');
                      }
            ...
            ) : currentUser.role?.toLowerCase() == 'approver' &&
              marketRecord?.status?.toLowerCase() == 'verified' ? (
              <div className="border-t-1 p-2 border-b-1 items-center text-center">
                <div className="p-2 mb-1">
                  Kindly approve or reject this report
                </div>
                <div className="flex justify-between mx-4 ">
                  <button
                    onClick={() => {
            ...
            ) : currentUser.role?.toLowerCase() == 'header' &&
              marketRecord?.status?.toLowerCase() == 'approved' ? (
              <div className="border-t-1 p-2 border-b-1 items-center text-center">
                <div className="p-2 mb-1">
                  Kindly forward or reject this report
                </div>
                <div className="flex justify-between mx-4 ">
                  <button
                    onClick={() => {
                      ...
                      >
                        ...
                        {viewers?.map((viewer, index) => (
                          <option
                            key={index}
                            value={viewer.id}
                          >{`${viewer.firstname} ${viewer.lastname}`}</option>
                        ))}
                      </select>
                      <button
                        ...
                        {selectedViewerIds.map((viewer, index) => (
                          ...
                          >
                            ...
                            <button
                              ...
                              className="text-red-500 font-bold px-2 focus:outline-none"
                            >
                              X
                            </button>
                          </div>
          ...
          {!commentLoading && (
            <div className="flex flex-wrap mb-2">
              {comments?.length > 0 &&
              !['viewer', 'minister'].includes(
                currentUser.role?.toLowerCase()
              ) ? (
                comments.map((comment, index) => (
                  <div
                    key={index}
                    className="w-full sm:w-1 md:w-1/2 lg:w-1/3 mb-4 p-4"
                  >
                  ...
                  .filter(
                    (comment) =>
                      comment.commented_by?.position?.toLowerCase() === 'mayor'
                  )
                  .map((comment, index) => comment).length < 1 ? (
                ...
                comments
                  ...
                  .map((comment, index) => (
                    <div
                      key={index}
                      className="w-full sm:w-1 md:w-1 lg:w-1/2 mb-4 p-4"
                    >
                      <div className="bg-gray-100 rounded-md p-4 text-sm ">
                        <p className="text-gray-700 mb-2">
                          <span className="font-bold">Commented By:</span>{' '}
        ...
        <div className="container mx-auto px-4 py-8">
          <form onSubmit={handleSubmit}>
            <div className="mb-4 flex">
              ...
              <div className="ml-8">
                ...
                <select
                  id="season"
                  name="season"
                  value={marketRecord?.season}
                  onChange={handleChange}
                  className="appearance-none border rounded py-2 px-8 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                >
                  <option value="">Select season</option>
                  {quaterList?.map((quater, index) => (
                    <option key={index} value={quater}>
                      {quater}
                    </option>
                  ))}
                </select>
            ...
            {marketRecord.records.map((record, index) => (
              <div
                key={index}
                className="flex flex-wrap items-center mb-4 px-4  py-1 border border-gray-300 justify-between rounded"
              >
                <label className=" text-gray-700 font-bold mb-2 mr-1">
                  {index + 1}
                </label>

                <div>
                  <label
                    htmlFor={`component_name_${index}`}
                    className="block text-gray-700 text-xs font-bold mr-2"
                  >
                    Component Name
                  </label>
                  <select
                    ...
                    onChange={(e) => {
                      if (
                        marketRecord.records.some(
                          (record) => record.component_name === e.target.value
                        )
                      )
                  ...
                  >
                    ...
                    {components.map((component) => (
                      <option
                        ...
                        data-available={component.total_number_places_available}
                      >
                        {component.name}
                      </option>
                    ))}
                ...
                <div className="items-center mb-2">
                  <label
                    htmlFor={`total_number_places_available_${index}`}
                    className="block text-gray-700 font-bold text-xs mr-2"
                  >
                ...
                <div className="items-center mb-2">
                  <label
                    htmlFor={`number_places_rented_${index}`}
                    className="block text-gray-700 font-bold text-xs mr-2"
                  >
                    Number of Places Rented
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleRecordChange(index, 'observation', e.target.value)
                    }
